...
docker pull "docker.pkg.github.com/elpassion/flounder/${app}" || true

docker build --build-arg VERSION="${version}" \
  -t "${ECR_REPO}" \
  -f "${workdir}/apps/${app}/Dockerfile" "${workdir}" \
  --platform linux/amd64 \
  --cache-from "docker.pkg.github.com/elpassion/flounder/${app}"
...
docker tag "${ECR_REPO}" "docker.pkg.github.com/elpassion/flounder/${app}" && docker push "docker.pkg.github.com/elpassion/flounder/${app}" || true
