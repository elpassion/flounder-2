...
import { isNil, isNotNil, Maybe } from '@flounder/ts-utils';
...
export class TestingAppRunner {
  ...
  async start(): Promise<void> {
    ...
    let moduleBuilder: TestingModuleBuilder = await createTestingModule({ imports: this.modules });
  ...
  getMockedProvider<T extends object>(provider: new (...args: any[]) => T): T {
    const providerMock = this.providerMocks.find(
      providerMock => providerMock.provider.name === provider.name,
    );
