const mockAsyncStorage = {
  ...
  setItem: async (key, value, callback) => {
    const setResult = await mockAsyncStorage.multiSet([[key, value]], undefined);
  ...
  getItem: async (key, callback) => {
    const getResult = await mockAsyncStorage.multiGet([key], undefined);
  ...
  removeItem: (key, callback) => mockAsyncStorage.multiRemove([key], callback),
  mergeItem: (key, value, callback) =>
    mockAsyncStorage.multiMerge([[key, value]], callback),
...
async function _multiSet(keyValuePairs, callback) {
  keyValuePairs.forEach((keyValue) => {
    ...
    mockAsyncStorage.__INTERNAL_MOCK_STORAGE__[key] = keyValue[1];
  });
...
async function _multiGet(keys, callback) {
  const values = keys.map((key) => [
    key,
    mockAsyncStorage.__INTERNAL_MOCK_STORAGE__[key] || null,
  ]);
...
async function _multiRemove(keys, callback) {
  keys.forEach((key) => {
    if (mockAsyncStorage.__INTERNAL_MOCK_STORAGE__[key]) {
      delete mockAsyncStorage.__INTERNAL_MOCK_STORAGE__[key];
    }
  });
...
async function _clear(callback) {
  mockAsyncStorage.__INTERNAL_MOCK_STORAGE__ = {};
...
async function _getAllKeys() {
  return Object.keys(mockAsyncStorage.__INTERNAL_MOCK_STORAGE__);
}
...
async function _multiMerge(keyValuePairs, callback) {
  keyValuePairs.forEach((keyValue) => {
    ...
    const oldValue = JSON.parse(mockAsyncStorage.__INTERNAL_MOCK_STORAGE__[key]);

    mockAsyncStorage.__INTERNAL_MOCK_STORAGE__[key] = JSON.stringify(
