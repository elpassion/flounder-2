...
import { Button, InputContainer } from '@flounder/ui';
...
export const SendEmailTemplateForm = () => {
  const intl = useIntl();
  const methods = useForm<MailExampleDto & { global: string }>({
    resolver: classValidatorResolver(MailExampleDto),
    defaultValues: {
      to: '',
      subject: '',
      text: '',
  ...
  const onSubmit: SubmitHandler<MailExampleDto> = data => mutate(data);
